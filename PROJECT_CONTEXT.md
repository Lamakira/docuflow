...
- **Platform-managed files**: `server/vite.ts` is still Replit-template code, but dev-only — `server/index.ts:34` imports it dynamically and the container never loads it, so it leaves with the dev server, not with the container. `vite.config.ts` is no longer template-managed: #25's `Dockerfile` copies it in as a build input and depends on its `outDir` (`dist/public`, which `server/static.ts` then serves), so edits here are this repository's. `package.json` and `drizzle.config.ts` likewise: the test runner (#27) and the `--env-file-if-exists=.env` flags on `dev`/`start` are edits made on purpose.
- **The container** (#25): one image for every runtime, `docs/CONTAINER.md` is the build-and-run reference. Its `Dockerfile` names its inputs one by one and `.dockerignore` is an allow-list, so **a new top-level build input needs a line in each** or the `COPY` fails. CI (`.github/workflows/ci.yml`) typechecks, builds, tests against a PostgreSQL service container, and builds the image on every push and pull request; nothing is published anywhere.
- **Env vars**: all read in `server/config.ts` and nowhere else. `.env.example` is the register of record and `docs/CONFIGURATION.md` explains each one — including which are required to boot, so this file does not carry a fourth copy of that list. The one shape worth knowing before you read either: `JWT_SECRET` is written `<key-id>:<secret>` and has no generated fallback (#23).
