...
- **Env vars**: all read in `server/config.ts` and nowhere else; every one documented in `.env.example` and `docs/CONFIGURATION.md`. Required to boot: database (`DATABASE_URL` or `PG*`), `SESSION_SECRET`, `PRIVATE_OBJECT_DIR`, `PUBLIC_OBJECT_SEARCH_PATHS`.
