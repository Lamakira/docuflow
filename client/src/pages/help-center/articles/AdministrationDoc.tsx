...
export function AdministrationDoc() {
  ...
  return (
    <div className="space-y-2">
      {!isAdmin ? (
        <Alert className="mb-6 border-muted">
          ...
          <AlertDescription className="text-xs sm:text-sm text-muted-foreground">
            The <strong className="text-foreground">Administration</strong> sidebar entry is only shown for accounts
            with the <code className="text-xs bg-muted px-1 py-0.5 rounded">admin</code> role. The sections below describe
            what administrators configure so you know why desktop behaviour or screenshots change organisation-wide.
          </AlertDescription>
        </Alert>
      ) : null}

      <DocSection title="Organisation policy and the desktop agent">
        <DocP>
          ...
          <strong className="text-foreground">next heartbeat</strong> (heartbeat runs on a fixed interval on the order
          of <strong className="text-foreground">one minute</strong>), so allow up to about one minute before assuming
          a change failed. This is current application behaviour.
        </DocP>
      </DocSection>

      <DocSection title="Screenshot capture policy (stable fields)">
        <DocP>
        ...
        <DocList>
          ...
          <DocLi>
            <strong className="text-foreground">activeHoursEnabled</strong> with <strong className="text-foreground">activeHoursStart</strong>{" "}
            and <strong className="text-foreground">activeHoursEnd</strong> — when enabled, restricts capture to a daily
            time window using 24-hour <code className="text-xs bg-muted px-1 py-0.5 rounded">HH:mm</code> values.
          </DocLi>
        </DocList>
        <DocScreenshotPlaceholder>
          [ Screenshot required: Admin web — Screenshot capture policy card with enable switch and intervals ]
        </DocScreenshotPlaceholder>
      </DocSection>

      <DocSection title="Idle behaviour policy (stable fields)">
        <DocP>
        ...
        <DocList>
          ...
          <DocLi>
            <strong className="text-foreground">idleTimeoutMinutes</strong> — minutes without qualifying activity before
            a prompt (allowed range in the admin form: 1–60).
          </DocLi>
          <DocLi>
            <strong className="text-foreground">idleCountdownSeconds</strong> — countdown length before automatic stop
            when the flow uses it (allowed range in the admin form: 15–120 seconds).
          </DocLi>
        </DocList>
        <DocScreenshotPlaceholder>
          [ Screenshot required: Admin web — Idle behaviour card with enable switch, timeout, and countdown fields ]
        </DocScreenshotPlaceholder>
        <DocP>
      ...
      <DocSection title="What admin settings affect for end users">
        <DocList>
          ...
          <DocLi>
            Other Administration pages (users, CRM modules, analytics) change who can log in, what metadata is
            required on CRM objects, and what admins see in dashboards — outside the narrow timer policy above, refer to
            your internal admin runbooks until those areas have dedicated Help Center depth.
          </DocLi>
        </DocList>
      </DocSection>

      <DocSection title="Screencasts timezone list (optional admin control)">
        <DocP>
      ...
      {isAdmin ? (
        <DocSection title="User management (overview)">
          <DocH3>Roles</DocH3>
          <DocP>
            Admins invite or manage users and assign the <code className="text-xs bg-muted px-1 py-0.5 rounded">admin</code>{" "}
            role where appropriate. Detailed invite flows stay in the Administration UI; this Help Center article does
            not replace your organisation&apos;s access policy.
          </DocP>
        </DocSection>
      ) : (
        <DocSection title="If you need an admin change">
          <DocP>
