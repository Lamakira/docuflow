...
try {
  ...
  const packed = fs.existsSync(outDir) &&
    fs.readdirSync(outDir).find((n) => n.endsWith("-linux-x64") && !n.startsWith("."));
  if (!packed) {
    console.error("[dist-linux] ERROR: electron-forge package failed:", err.message);
    process.exit(1);
  }
  console.log("[dist-linux] Note: forge hung at postPackage hook but output is present — continuing.");
}
