...
console.log(`[dist-win] target host: ${process.env.DOCUFLOW_DEFAULT_API_URL}`);
...
try {
  ...
  const packed = fs.existsSync(outDir) &&
    fs.readdirSync(outDir).find((n) => n.endsWith("-win32-x64") && !n.startsWith("."));
  if (!packed) {
    console.error("[dist-win] ERROR: electron-forge package failed:", err.message);
    process.exit(1);
  }
  console.log("[dist-win] Note: forge hung at postPackage hook but output is present — continuing.");
}
