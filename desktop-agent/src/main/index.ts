...
function createWidgetWindow(): BrowserWindow {
  ...
  win.webContents.on("did-finish-load", () => {
    ...
    // Seed the freshly-loaded widget and decide its visibility.
    //
    // syncWidgetVisibility is otherwise only reached through
    // pushStateToRenderer, which is guarded by `if (widgetWindow)`. At startup
    // the session restore pushes state before this window finishes loading, so
    // a restored running/paused timer left the activity bar hidden for the
    // whole session — it only appeared once the timer was next touched.
    const timerState = store.getTimerState();
    win.webContents.send("widget:state-update", { timer: timerState });
...
/**
 * Tray icon for a timer status. The tray is often the only visible part of the
 * app, so the icon answers "am I tracking?" on its own: idle is grey, running
 * blue, paused marigold. Generated by scripts/gen-tray-icons.js.
 */
function getTrayIconPath(status?: string): string {
  const suffix = status === "running" ? "-running" : status === "paused" ? "-paused" : "";
  const file = `tray-icon${suffix}.png`;
...
/** Load a tray image, downscaling for the ~22pt macOS menu bar. */
function loadTrayImage(status?: string) {
  let image = nativeImage.createFromPath(getTrayIconPath(status));
...
function rebuildTrayMenu(): void {
  ...
  // Runs on every state push, so the icon tracks the timer without extra wiring.
  const icon = loadTrayImage(isPaired ? timerStatus : undefined);
  if (!icon.isEmpty()) tray.setImage(icon);
  tray.setToolTip(
    !isPaired
      ? "DocuFlow — not signed in"
      : timerStatus === "running"
        ? "DocuFlow — tracking"
        : timerStatus === "paused"
          ? "DocuFlow — paused"
          : "DocuFlow — not tracking",
  );
...
function createTray(): void {
  try {
    ...
    const image = loadTrayImage(store.getTimerStatus());
...
/**
 * Window size for the renderer that is actually running.
 *
 * The window is created at v1's portrait 680×780. The v2 rail layout is three
 * columns and needs a landscape window, but which UI boots is a renderer-side
 * decision (localStorage, then the DOCUFLOW_UI env var), so the renderer asks
 * for its own size once it knows. A window the user has already made big enough
