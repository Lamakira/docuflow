...
**The residency question is a blocking input to cutover, not to this publish.** Whoever owns go-to-market answers where DocuFlow's data subjects live before traffic switches. While the environment holds only synthetic data a wrong geography costs a rebuilt project; after cutover the same correction is a production data migration inside a freeze window, because ADR-0018 makes this environment the successor rather than a scratch copy. [#61](https://github.com/Lamakira/docuflow/issues/61) carries the question into the phase record so it is not found at the gate.
...
| Cutover ([ADR-0018](../adr/0018-build-in-a-parallel-environment-with-snapshot-rehearsed-cutover.md)) | The residency answer ADR-0024 gates on: where DocuFlow's data subjects live. Unanswered at that gate stops the cutover, because correcting the geography afterwards is a data migration rather than a rebuild |
