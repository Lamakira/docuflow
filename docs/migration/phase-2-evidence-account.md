...
[ADR-0021](../adr/0021-run-the-compute-on-replit-and-supersede-the-render-hosting-decision.md) raises the stakes since #57 was written. Its point-in-time-restore amendment is **Contested** — the Pro billing page advertises 28-day PITR while the database reference attributes PITR to legacy infrastructure and gives current databases a checkpoint rollback instead. Until that is read from the dashboard, **these exports may be the recovery point of record**, not a secondary copy. This is the ticket that decides whether losing the database is survivable, and it should not be scheduled as though it were paperwork.
