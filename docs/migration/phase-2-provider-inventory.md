...
- **Corrected 2026-08-13 by [#53](https://github.com/Lamakira/docuflow/issues/53).** Four statements below did not survive a second reading of Replit's documentation and are marked where they appear: the **PITR gate is now Contested rather than Answered**, development environments are **not** unconditionally North American, the production database is **not** necessarily Neon, and `PG*` variables are not provided on Replit's current infrastructure. The corrections and their sources are in [`phase-2-deployments-and-databases.md`](phase-2-deployments-and-databases.md); the original text stands unedited beneath each note, in the convention the ADRs use.
- **Corrected 2026-08-18 by [#53](https://github.com/Lamakira/docuflow/issues/53).** The parallel Replit project is **published** to **Europe (EU)** at `https://docuflow-masdouk1.replit.app`. Host suffixes invert the table's original assignment: **production is Helium**, **development is Neon**. PITR on production therefore follows the Helium (checkpoint) branch, and [#57](https://github.com/Lamakira/docuflow/issues/57)'s nightly exports are the recovery point of record. The Replit row below should be read as provisioned; the Neon/Helium rows should be read swapped.
...
- **Amended 2026-09-17 by [#230](https://github.com/Lamakira/docuflow/issues/230).** Sign-up on the Clerk application #107 provisioned reads `sign_up.mode: "public"` — open. Phase 5 ([#110](https://github.com/Lamakira/docuflow/issues/110)) had asked for it to be disabled and that was never applied; the instruction is now withdrawn, because [#217](https://github.com/Lamakira/docuflow/issues/217) removed the dead end it protected against. No dashboard change was made. See [`phase-8-self-service-registration.md`](phase-8-self-service-registration.md).
- **Corrected 2026-09-02 by [#107](https://github.com/Lamakira/docuflow/issues/107).** The Clerk row is **Provisioned** (Hobby). The account is DocuFlow-owned; it is not Replit Auth's tenant (ADR-0021). Signup offered no region picker: Clerk hosts identity in the US. Login is unchanged until Phase 5 cutover.
...
| **Clerk** (DocuFlow-owned) | The managed identity provider behind ADR-0007's `IdentityProvider` seam | **Provisioned** ([#107](https://github.com/Lamakira/docuflow/issues/107)) — DocuFlow-owned Hobby application; not Replit Auth's tenant ([ADR-0021](../adr/0021-run-the-compute-on-replit-and-supersede-the-render-hosting-decision.md)) | — | **US-hosted.** Signup had no region picker. Clerk does not offer EU identity residency on Hobby. ADR-0007 already assumed US-first; reassess only for an explicit EU-residency customer | Hobby | — | — |
...
| The Clerk credentials | Secret | Clerk | **[#107](https://github.com/Lamakira/docuflow/issues/107):** `CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`. Names only — values stay in the dashboard / secret store. [`server/config.ts`](../../server/config.ts) does not read them yet; they replace the Replit OIDC row above at Phase 5 cutover ([#110](https://github.com/Lamakira/docuflow/issues/110)) |
...
> **Corrected 2026-08-13 (#53).** "Production is Neon" is not documented for an app created on Replit's current infrastructure. Replit's database reference says it creates the production database at publish "for you when needed" and treats Neon as the **legacy** operator that development databases left on 2025-12-04; which operator a published database actually runs is read from its host suffix and is recorded in [`phase-2-deployments-and-databases.md`](phase-2-deployments-and-databases.md), not assumed here. The `Neon (production database)` row above should be read as "the production database, operator unread".
...
**`DB_DRIVER` may not be the same value in both environments.** The default is Neon's serverless driver, which reaches PostgreSQL over WebSockets; `pg` selects node-postgres for a database that driver cannot reach. Whether Helium accepts the Neon driver is a question for #53, alongside the pooling and `-pooler` hostname question it already carries.

> **Answered 2026-08-13 (#53): it is the same value in both, and that value is `pg`.** Running the app against the App's Replit-hosted development database required node-postgres — the serverless driver's WebSocket transport does not reach it — and node-postgres reaches a Neon database over plain TLS as well, so one setting covers both operators. It is now in [`.replit`](../../.replit)'s `[env]` block, which also settles the observability edge below in favour of having spans. It also has an observability edge: [`docs/OBSERVABILITY.md`](../OBSERVABILITY.md) records that the Neon driver is uninstrumented, so query spans appear under `DB_DRIVER=pg` and not under the default. Two environments on different drivers therefore produce different traces.

> **Corrected 2026-08-13 (#53).** The premise of the paragraph below — development is North American regardless — is not what Replit documents. **Workspace geography is selectable by Pro customers** (North America, Europe (EU), Asia) and is fixed at workspace creation, separately from the publishing geography chosen at first publish; the two "do not need to match". The rule below survives on isolation grounds in every case and on residency grounds only for a workspace actually created outside the published geography, which is a per-project observation and not a platform constant. Record it before treating either as given.
...
ADR-0021 leaves three questions gating execution. Two are procurement questions that #52 owns. Both are now closed, by different means: the PITR gate is **Answered** from Replit's own documentation, and the Clerk gate is **Resolved by decision** without its question being put. The third — whether one Replit project can run an HTTP deployment and a Reserved VM worker deployment simultaneously — is a deployment-topology question owned by [#53](https://github.com/Lamakira/docuflow/issues/53), and is named here only so that a reader counting gates finds all three. **It is now answered, no**: deployment type is a property of the App, the `.replit` reference holds one `[deployment]` table, and the worker therefore gets a second Replit project on this same repository — see [`phase-2-deployments-and-databases.md`](phase-2-deployments-and-databases.md), which also records why that project differs by a variable rather than by an edited `.replit`.
...
> **Reopened 2026-08-13 (#53): Contested.** Read again a day later, Replit's documentation says two incompatible things. The Pro plan page still advertises restore "to any point in time within the last 28 days, instead of the standard 7 days" — the source of the answer below. The database reference now attributes point-in-time restore to the **legacy Neon** infrastructure and gives current Replit-hosted **Helium** databases a rollback-to-Agent-checkpoint feature instead, which is not a point in time. Both are provider statements; which governs depends on the operator the published production database turns out to run, and that cell is filled by #53's publish. Until it is, ADR-0016's five-minute RPO is unevidenced on this platform and [#57](https://github.com/Lamakira/docuflow/issues/57)'s nightly exports may be the recovery point of record after all. The answer below stands unedited as what was read on 2026-08-12.

> **Settled 2026-08-18 (#53): production is Helium.** The database-reference branch governs. PITR is not the recovery control here. [#57](https://github.com/Lamakira/docuflow/issues/57)'s nightly exports **are** the recovery point of record. Development is Neon — the inverse of the table above.
...
- **Phase 5 owns the purchase, #52 owns the registration.** The account is registered in the inventory now so Phase 5 does not discover it. Hobby was provisioned 2026-09-02 under [#107](https://github.com/Lamakira/docuflow/issues/107).
- **[#58](https://github.com/Lamakira/docuflow/issues/58) does not list Clerk.** Its spend-alert providers are Replit, Cloudflare/R2, AWS, Better Stack, Sentry, OpenAI, and Azure Speech. This decision creates a Clerk bill that no alert currently covers. That list is doubly stale now: ADR-0023 also removes R2, and folds object-storage spend into the Replit line. Hobby is $0 until 50,000 MRU or a paid feature is required.
- **Identity residency is now the odd one out and is recorded.** Every other provider in this inventory is EU-pinned — Replit EU, App Storage by geography inheritance, AWS Frankfurt, Better Stack EU, Sentry Germany. Signup for this Clerk application offered no region picker; Clerk hosts identity in the US (lawful transfer via DPF/DPA, not in-region storage). That matches ADR-0007's US-market-first launch. An explicit EU-residency or enterprise-procurement requirement still triggers reassessment before onboarding that customer.
