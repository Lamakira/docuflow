...
| `GCS_SERVICE_ACCOUNT_KEY` **or** `GOOGLE_APPLICATION_CREDENTIALS`, plus `GCS_PROJECT_ID` | Secret (the first two); `[env]` for `GCS_PROJECT_ID` | Replit App Storage | [#59](https://github.com/Lamakira/docuflow/issues/59) — names are **already fixed**, see below |
...
- **The storage credential is no longer an open name.** This was the larger of the two gaps and [ADR-0023](../adr/0023-prefer-replit-provided-capability-wherever-an-exit-can-be-proved.md) closes it rather than moving it. Replit App Storage is Google Cloud Storage-backed and reachable through the `@google-cloud/storage` client [`server/objectStorage.ts`](../../server/objectStorage.ts) already uses, so no S3-shaped adapter is written and the GCS-shaped variables that exist today are the ones the bucket occupies: `GCS_SERVICE_ACCOUNT_KEY` or `GOOGLE_APPLICATION_CREDENTIALS`, plus `GCS_PROJECT_ID`. ADR-0022's accepted cost — the provider swap moving into Phase 2 as code — is withdrawn with it. The bucket-root form is unaffected: ADR-0022 makes the bucket name and prefix configuration rather than identity, which is what lets a restored database resolve against a differently named bucket, and it names `PRIVATE_OBJECT_DIR` doing so, since `normalizeObjectEntityPath` reduces uploads to `/objects/{entityId}` beneath it. `PUBLIC_OBJECT_SEARCH_PATHS` is the same shape and ADR-0022 does not name it, so treat its survival as expected rather than decided. **One question is open and belongs to #59**: whether that client can reach an App Storage bucket with a credential we hold, or only through Replit-injected authentication. ADR-0023 accepted the adoption on an exit that is list-then-download, and if the credential is Replit-injected then that exit runs on the platform being exited.
...
| [#59](https://github.com/Lamakira/docuflow/issues/59) | The App Storage bucket on the Replit project, and the answer to whether the GCS client can reach it with a credential we hold |
