...
| `@clerk/backend`        | `tests/fakes/clerk.ts`   | Users created from a bcrypt digest, password verify, and session-token verify in memory. CI never reaches api.clerk.com. The IdentityProvider port fake is `tests/fakes/identityProvider.ts`. |
...
All six vendor fakes are module-level singletons; `tests/setup.ts` resets them in a
