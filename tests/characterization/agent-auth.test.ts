 ...
 *
 * `docs/agent-protocol.md` still describes the flow this replaced, so the doc is
 * not the contract; these assertions are. Divergences from the document, all
 * frozen below:
 *
 *  - §3/§4.1 describe a pairing-code exchange. Both pairing endpoints answer
 ...
 *  - Every login creates another device row; nothing reuses a machine's device.
...
describe("desktop agent auth and devices (characterization)", () => {
  ...
  it("answers 410 on both retired pairing endpoints, whatever is posted", async () => {
    const app = await makeApp();
    ...
    expect(start.body).toEqual({
      message:
        "Pairing codes have been removed. Use the desktop app and sign in with your DocuFlow email and password.",
    });

    // Quirk: neither endpoint authenticates or validates first — the 410 is the
    // whole handler, so a stale agent gets the same answer with any payload.
    const complete = await newAgent(app)
      .post("/api/agent/pairing/complete")
      .send({ pairingCode: "A7K3M9", deviceMeta: { deviceName: "Old Agent" } });
    expect(complete.status).toBe(410);
    expect(complete.body).toEqual(start.body);

    const unauthenticatedStart = await newAgent(app).post("/api/agent/pairing/start").send({});
    expect(unauthenticatedStart.status).toBe(410);
  });
